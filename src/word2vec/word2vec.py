...
def getNegativeSamples(outsideWordIdx, dataset, K, name):
    """ Samples K indexes which are not the outsideWordIdx """

    negSampleWordIndices = [None] * K
    for k in range(K):
        newidx = dataset.sampleTokenIdx(name)
...
def negSamplingLossAndGradient(
    ...
    negSampleWordIndices = getNegativeSamples(outsideWordIdx, dataset, K, name)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        gradloss, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset, 10, name)
        loss += gradloss
...
def word2vec_sgd_wrapper(word2vecModel, word2Ind, wordVectors, dataset,
    ...
    for i in range(batchsize):
        ...
        centerWord, context = dataset.getRandomContext(windowSize1, name)

        c, gin, gout = word2vecModel(
            centerWord, windowSize1, context, word2Ind, centerWordVectors,
            outsideVectors, dataset, word2vecLossAndGradient, name
        )
        loss += c / batchsize
