...
class StanfordSentiment:
    def __init__(self, path=None, tablesize = 1000000):
        if not path:
            path = "src/word2vec/utils/datasets"
    ...
    def tokens(self, name):
        ...
        for sentence in self.sentences(name):
            for w in sentence:
                wordcount += 1
                if not w in tokens:
                    tokens[w] = idx
    ...
    def sentences(self, name):
        ...
        sentences = []
        file_queue = [name] if name != "all" else [f"{i}star" for i in range(1, 5+1)]
        for j in file_queue:
            with open(self.path + f"/{j}.csv", "r") as f:
                first = True
                for line in f:
                    if first:
                        first = False
    ...
    def allSentences(self, name):
        if hasattr(self, "_allsentences") and self._allsentences:
            return self._allsentences

        sentences = self.sentences(name)
        rejectProb = self.rejectProb(name)
    ...
    def getRandomContext(self, C, name):
        allsent = self.allSentences(name)
        ...
        if len(context) > 0:
            ...
            return self.getRandomContext(C, name)
    ...
    def sampleTable(self, name):
        if hasattr(self, '_sampleTable') and self._sampleTable is not None:
            return self._sampleTable

        nTokens = len(self.tokens(name))
        samplingFreq = np.zeros((nTokens,))
        self.allSentences(name)
        i = 0
    ...
    def sampleTokenIdx(self, name):
        return self.sampleTable(name)[random.randint(0, self.tablesize - 1)]